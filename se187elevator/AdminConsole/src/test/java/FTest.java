...
public class FTest extends TestCase {
	...
	// FloorPanelUI fp= new
	// FloorPanelUI(fi,FloorPanelColor.BLUE,FloorPanelColor.RED,1,"uptext","downText");
	...
	/*
	 * public void testdisableUpButton(){ fpUI.disableUpButton();
	 * if(fpUI.UpButton.isEnabled()) { output = "true"; } else { output =
	 * "false"; }
	 * 
	 * assertEquals("false",output); }
	 * 
	 * /*public void testprocessedRequest(){ fpUI.processedRequest(); Color
	 * opcolor = fpUI.UpButton.getBackground(); Color ipcolor = new
	 * Color(255,255,0); if(opcolor.getRGB() == ipcolor.getRGB()) { output =
	 * "pass"; } assertEquals("pass",output); }
	 * 
	 * public void testbuttonStatus(){
	 * fpUI.floorPanel.setFloorPanelStatus("Active");
	 * assertEquals("Active",fpUI.floorPanel.getFloorPanelStatus()); }
	...
	public void testrequestEnqueue() {
		...
		assertEquals("[currentFloornumber is 1 and direction is UP]",
				fpQ.queueFloorPanelRequest.toString());
	}

...
public class ElevatorSystemTest extends TestCase{
	...
	public class testList extends FloorPanelQueue{
		...
		public void testQueueList(FloorPanelQueue queue){
			...
			LinkedList<FloorPanelRequest> queueFloorPanelRequest = queue.getQueueFloorPanelRequest();
			
			Car car = new Car();
			for(int i=1;i<=10;i++){
				queueFloorPanelReques.offer(new FloorPanelRequest(i, Direction.UP));
			}
			
			while(queueFloorPanelRequest.size()!=0){
				System.out.println("List1..."+queueFloorPanelReques.pollFirst().toString());
				System.out.println("List..."+queue.queueFloorPanelRequest.pollFirst().toString());
	...
	/*public class testList extends FloorPanelQueue{
		...
		public void testQueueList(FloorPanelQueue queue){
			...
			while(queueFloorPanelRequest.size()!=0){
				...
				assertEquals(queue.queueFloorPanelRequest.toString(),queueFloorPanelReques.toString());
			}
		}
	}*/
		...
		public void testUserPanelFloorEnqueue(){
			...
			for(int i=1;i<=10;i++){
				...
				System.out.println("Up Queue Contains: "  +queue.queueUserPanelRequestUp);
				System.out.println("Down Queue Contains: "+queue.queueUserPanelRequestDown);
		...
		public void testQueueEmpty(){
			UserPanelQueue queue = new UserPanelQueue();
			LinkedList<UserPanelRequest> queueUserPanelRequestUp = queue.getQueueUserPanelRequestUp();
		...
		public void testQueueEmptyEnqueueing(){
			...
			if(queue.queueUserPanelRequestUp.size()==0 && queue.queueUserPanelRequestDown.size()==0)
			for(int i=1;i<=10;i++){
				car.getUserPanelQueue().putMessage(i);
			}
			
			System.out.println("Queue Size Up: "+queue.queueUserPanelRequestUp.size());
			System.out.println("Queue Size Down: "+queue.queueUserPanelRequestDown.size());
